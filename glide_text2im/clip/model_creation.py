    ...
    def cond_fn(self, prompts: List[str], grad_scale: float) -> Callable[..., torch.Tensor]:
        ...
        def cond_fn(x, t, grad_scale=grad_scale, **kwargs):
            with torch.enable_grad():
                ...
                similarity=(z_t * z_i).sum()#
                arr_.append(similarity.detach().cpu().item())
                #print(similarity.detach().cpu().item()) #
                loss = torch.exp(self.logit_scale) * similarity
                grad = torch.autograd.grad(loss, x_var)[0].detach()
               # print(t)
                if t.detach().cpu()[0]==0:
                    x_ = np.array(list(range(len(arr_))))
                    y = np.array(arr_)
                    plt.plot(x_,y)
                    plt.xlabel('time')
                    plt.ylabel('similarity')
                    plt.title('similarity per time')
                    plt.show()
